...
static uint32_t verification_start_tick = 0; /* 台上验证延时计时器 */
    ...
    for (int i = 0; i < 4; i++)
    {
      ...
      if (motor_target_speed[i] == 0.0f) {
          ...
          if (is_vision_debug_mode == 1) {
              ...
              PID_Clear(&motor_pid[i]);
          } else if (robot_state == ROBOT_CLIMBING_OFFSTAGE) {
              /* 台下闭环登台阶段：旁路电机速度 PID，输出直接等于带角度纠偏的目标速度以保证最大出力 */
              output = motor_target_speed[i];
              PID_Clear(&motor_pid[i]);
          } else {
...
void StartMotion_Task(void *argument)
{
  ...
  for(;;)
  {
    ...
    /* 移除了底盘灰度渐变滤波与平复检测，改由高低区间与几何检测直接确诊 */
    ...
    {
      /* 增加台下自动上台状态下的“直感台上检测”自愈：
         只在 ROBOT_SCANNING 状态，或者 ROBOT_CLIMBING_OFFSTAGE 状态且倒车爬行时间已超过 500ms 时起作用。
         可通过 4 路底盘灰度传感器，或几何开阔空间特征（红外测距）判定已在台上。 */
      uint8_t allow_onstage_detect = 0;
      if (robot_state == ROBOT_SCANNING)
      {
        allow_onstage_detect = 1;
      }
      else if (robot_state == ROBOT_CLIMBING_OFFSTAGE)
      {
        if (HAL_GetTick() - climb_start_tick >= 500U)
        {
          allow_onstage_detect = 1;
        }
      }
      
      if (allow_onstage_detect)
      {
        ...
        uint8_t geometry_onstage = (IR_Distance_UP >= TOP_IR_THRESHOLD && IR_Distance_B >= 60.0f && (IR_Distance_L >= 50.0f || IR_Distance_R >= 50.0f));
        
        if (low_grey_count >= 3 || geometry_onstage)
        {
          Motor_Control(0, 0);
          robot_state = ROBOT_RUNNING;
          onstage_confirmed = 1;
          printf("[AutoClimb] Direct Onstage Auto-Recovery! %s confirm we are already on stage. State -> ROBOT_RUNNING\r\n",
                 (low_grey_count >= 3) ? "Underbody sensors" : "Geometry sensors (IR Dist)");
          osDelay(20);
          continue;
        }
      }
      switch (robot_state)
      {
        ...
        case ROBOT_INIT_CLIMB:
        {
          ...
          if (HAL_GetTick() - climb_start_tick >= 1000U)
          {
            ...
            if (low_grey_count >= 3)
            {
              ...
              robot_state = ROBOT_RUNNING;
              printf("[SoftClimb] Initial climb SUCCESS! Underbody sensors confirm we are on stage. State -> ROBOT_RUNNING (onstage).\r\n");
            }
            else
            {
              /* 没冲上去，判定在台下 */
              ...
              control_mode = 1;
              is_test_mode = 0;
              robot_state = ROBOT_TEST_ROTATE_PREPARE; // 退回270°扫描重新登台
              printf("[SoftClimb] Initial climb FAILED! Underbody sensors show we are still off-stage. Transition to ROBOT_TEST_ROTATE_PREPARE.\r\n");
            }
          }
          break;
        }
        ...
        case ROBOT_ALIGN_ORTHO:
        {
          ...
          {
            /* 转向完成：停止电机。既然已经用 Yaw 对准了，直接在此处根据上方红外判定方向，无需自转扫掠！ */
            Motor_Control(0, 0);
            osDelay(200);
            
            if (IR_Distance_UP >= TOP_IR_THRESHOLD)
            {
              /* 屁股（后部）面对的是台阶，铲子正对围栏，直接前冲对齐 */
              robot_state = ROBOT_SQUARING;
              squaring_start_tick = HAL_GetTick();
              printf("[OrthoAlign] Yaw %.1f aligned. Butt faces stage (correct). Squaring...\r\n", target_angle_lock);
            }
            else
            {
              /* 反了，屁股面对的是高围栏，需要原地调头 180 度 */
              float opp_yaw = target_angle_lock + 180.0f;
              while (opp_yaw > 180.0f)  opp_yaw -= 360.0f;
              while (opp_yaw < -180.0f) opp_yaw += 360.0f;
              target_angle_lock = opp_yaw;
              
              corner_type = 1;   /* 借用脱困逻辑变量 */
              corner_phase = 1;  /* 直接进入旋转阶段 */
              robot_state = ROBOT_CORNER_ESCAPE;
              printf("[OrthoAlign] Yaw %.1f aligned but reversed (UP_IR = %.1f). Adjusting 180 deg to %.1f...\r\n", 
                     IMU_Data.Yaw, IR_Distance_UP, target_angle_lock);
            }
            init_rotate_pd = 0;
          }
          else
        ...
        case ROBOT_LAUNCH_DECIDE:
        {
          ...
          /* 启动决策：利用 几何距离(不含前红外) + 底盘灰度 融合双重锁进行判定，达到 100% 绝对可靠性。
             - 台上特征：
               ...
               2. 几何特征：由于擂台是 6cm 的凸起平台，水平测距传感器（左L、右R、后B）及上方红外会射向空中，读数极其开阔。
             - 台下特征：
               1. 灰度特征：身处纯黑地表通道，所有灰度传感器读数均较高（均 > 200.0f）。
               2. 几何特征：处于狭窄通道中（通道宽度仅约45cm），水平传感器必然会有多侧受限，无法满足开阔条件。
          */
          uint8_t low_grey_count = 0;
          if (Grey_Front < 180.0f)  low_grey_count++;
          if (Grey_Back < 180.0f)   low_grey_count++;
          ...
          uint8_t geometry_onstage = (IR_Distance_UP >= TOP_IR_THRESHOLD && IR_Distance_B >= 60.0f && (IR_Distance_L >= 50.0f || IR_Distance_R >= 50.0f));
          if (low_grey_count >= 3 || (low_grey_count >= 1 && open_sides >= 2) || geometry_onstage)
          {
            is_onstage = 1;
          }
          ...
          else
          {
            ...
            printf("[AutoClimb] Position: OFF-STAGE. Initiating 270 deg scan... State -> ROBOT_TEST_ROTATE_PREPARE\r\n");
          }
          break;
        }
        ...
        case ROBOT_SCANNING:
        {
          ...
          */
          ...
          /* 2. 动态扫掠过程中，由原先的红外差值判定，重构为用 MPU6050 的 Yaw 轴对齐 */
          else
          {
            float yaw_div = IMU_Data.Yaw / 90.0f;
            int rounded_index = (yaw_div >= 0.0f) ? (int)(yaw_div + 0.5f) : (int)(yaw_div - 0.5f);
            float nearest_snap_yaw = (float)rounded_index * 90.0f;
            float snap_err = IMU_Data.Yaw - nearest_snap_yaw;
            while (snap_err > 180.0f)  snap_err -= 360.0f;
            while (snap_err < -180.0f) snap_err += 360.0f;
            
            if (fabs(snap_err) < 5.0f)
            {
              /* 必须检测到有侧向或后方墙壁，表明在通道中 */
              if (IR_Distance_B < 50.0f || IR_Distance_L < 50.0f || IR_Distance_R < 50.0f)
              {
                target_angle_lock = nearest_snap_yaw;
                
                if (IR_Distance_UP >= TOP_IR_THRESHOLD)
                {
                  /* 完美！屁股（后部）面对的是低台阶（射空），铲子（前部）正对高围栏。直接进入顶墙对齐！ */
                  robot_state = ROBOT_SQUARING;
                  squaring_start_tick = HAL_GetTick();
                  Motor_Control(0, 0);
                  printf("[AutoClimb] Sweep Yaw aligned to %.1f. Squaring...\r\n", target_angle_lock);
                }
                else
                {
                  /* 反了！屁股（后部）面对的是围栏高墙，车头（前部）正对低台阶。我们需要原地旋转 180 度！ */
                  float opp_yaw = target_angle_lock + 180.0f;
                  while (opp_yaw > 180.0f)  opp_yaw -= 360.0f;
                  while (opp_yaw < -180.0f) opp_yaw += 360.0f;
                  target_angle_lock = opp_yaw;
                  ...
                  robot_state = ROBOT_CORNER_ESCAPE;
                  Motor_Control(0, 0);
                  printf("[AutoClimb] Sweep Yaw aligned to %.1f but reversed (UP_IR = %.1f). Adjusting 180 deg to %.1f...\r\n", 
                         IMU_Data.Yaw, IR_Distance_UP, target_angle_lock);
                }
                break;
              }
        ...
        case ROBOT_TEST_ROTATE_90:
        {
          ...
          if (fabs(yaw_err) < 3.0f)
          {
            ...
            if (rotate_step >= 3)
            {
              ...
              {
                printf("不在边缘且不在角落 (障碍数: %d)，小车重新回到台上运行状态以自愈\r\n", blocked_count);
                robot_state = ROBOT_RUNNING;
              }
            }
            else
        ...
        {
          /* 前进避开角落障碍：直行直到后上方红外距离大于 110.0f */
          Motor_Control(0, 25);
          
          uint32_t elapsed_escape = HAL_GetTick() - climb_wall_align_start_tick;
          if (IR_Distance_UP > 110.0f || elapsed_escape >= 2500U)
          {
            ...
            printf("[TestClimb] Corner escape forward complete (UP_IR=%.1f, elapsed=%lums). Next: turn to opposite of D_smaller (%.1f) and move forward until wall\r\n", 
                   IR_Distance_UP, elapsed_escape, climb_target_angle);
          }
          break;
        }
        ...
        case ROBOT_CLIMB_FORWARD_UNTIL_WALL:
        {
          ...
          /* 依据图片逻辑：前方红外测距大于 48 或者后方红外测距大于 60，启动 1 秒倒计时（增加 2500ms 超时） */
          if (IR_Distance_F > 48.0f || IR_Distance_B > 60.0f || elapsed_align >= 2500U)
          {
            /* 条件触发，开启 1 秒延时 */
        ...
        case ROBOT_CLIMB_BACKWARD_FAST:
        {
          ...
          if (HAL_GetTick() - climb_timer >= 1000U)
          {
            ...
            robot_state = ROBOT_RUNNING;
            onstage_confirmed = 0;
            printf("[AutoClimb] Onstage complete! Robot back on stage. State -> ROBOT_RUNNING\r\n");
          }
          break;
        }
    ...
    if (robot_state == ROBOT_RUNNING && is_test_mode == 0)
    {
      ...
      /* 10秒卡死及掉台监测历史数据变量声明 */
      static float yaw_hist[10] = {0.0f};
      static float ir_f_hist[10] = {0.0f};
      static float grey_hist[4][10] = {0};
      ...
      if (onstage_settle_start_tick == 0)
      {
        ...
        printf("[FallDetection] onstage running! Settle timer started (1s).\r\n");
      }
      
      /* 10秒无动作卡死自愈监测 */
      if (HAL_GetTick() - last_hist_tick >= 1000U)
      {
        last_hist_tick = HAL_GetTick();
        
        // 移动历史窗口
        for (int i = 9; i > 0; i--)
        {
          yaw_hist[i] = yaw_hist[i-1];
          ir_f_hist[i] = ir_f_hist[i-1];
          for (int j = 0; j < 4; j++)
          {
            grey_hist[j][i] = grey_hist[j][i-1];
          }
        }
        
        // 插入最新采样
        yaw_hist[0] = IMU_Data.Yaw;
        ir_f_hist[0] = IR_Distance_F;
        grey_hist[0][0] = Grey_Front;
        grey_hist[1][0] = Grey_Back;
        grey_hist[2][0] = Grey_Left;
        grey_hist[3][0] = Grey_Right;
        ...
        if (hist_count >= 10)
        {
          float y_min = yaw_hist[0], y_max = yaw_hist[0];
          float ir_min = ir_f_hist[0], ir_max = ir_f_hist[0];
          float g_min[4], g_max[4];
          for (int j = 0; j < 4; j++)
          {
            g_min[j] = grey_hist[j][0];
            g_max[j] = grey_hist[j][0];
          }
          
          for (int i = 1; i < 10; i++)
          {
            if (yaw_hist[i] < y_min) y_min = yaw_hist[i];
            if (yaw_hist[i] > y_max) y_max = yaw_hist[i];
            
            if (ir_f_hist[i] < ir_min) ir_min = ir_f_hist[i];
            if (ir_f_hist[i] > ir_max) ir_max = ir_f_hist[i];
            
            for (int j = 0; j < 4; j++)
            {
              if (grey_hist[j][i] < g_min[j]) g_min[j] = grey_hist[j][i];
              if (grey_hist[j][i] > g_max[j]) g_max[j] = grey_hist[j][i];
            }
          }
          
          float yaw_span = y_max - y_min;
          float ir_span = ir_max - ir_min;
          uint8_t grey_stable = 1;
          for (int j = 0; j < 4; j++)
          {
            if (g_max[j] - g_min[j] >= 12.0f)
            {
              grey_stable = 0;
              break;
            }
          ...
          /* 若偏航角、前红外测距及4路底盘灰度均高度稳定波动，判定卡死或掉台 */
          if (yaw_span < 10.0f && ir_span < 4.0f && grey_stable)
          {
            Motor_Control(0, 0);
            ...
            is_test_mode = 0;
            robot_state = ROBOT_TEST_ROTATE_PREPARE;
            printf("[StuckDetection] Stuck for 10s detected (Yaw span=%.1f, IR span=%.1f). Forcing 270 deg scan to recover...\r\n", yaw_span, ir_span);
            osDelay(20);
            continue;
          }
        }
      ...
      {
        /* 方案二：直感掉台检测（无 IMU 倾斜触发的平滑滑落） */
        uint8_t high_grey_count = 0;
        if (Grey_Front > 190.0f) high_grey_count++;
        if (Grey_Back > 190.0f)  high_grey_count++;
        if (Grey_Left > 190.0f)  high_grey_count++;
        if (Grey_Right > 190.0f) high_grey_count++;
        ...
        static uint32_t direct_fall_timer = 0;
        uint8_t geometry_onstage = (IR_Distance_UP >= TOP_IR_THRESHOLD && IR_Distance_B >= 60.0f && (IR_Distance_L >= 50.0f || IR_Distance_R >= 50.0f));
        if (high_grey_count >= 3 && !geometry_onstage)
        {
          if (direct_fall_timer == 0)
          {
            direct_fall_timer = HAL_GetTick();
          }
          else if (HAL_GetTick() - direct_fall_timer >= 500U)
          {
            Motor_Control(0, 0);
            ...
            is_test_mode = 0;
            robot_state = ROBOT_TEST_ROTATE_PREPARE;
            printf("[FallDetection] Direct Fall CONFIRMED (Grey >= 3 > 190 && !geometry_onstage for 500ms)! Transition to ROBOT_TEST_ROTATE_PREPARE...\r\n");
            osDelay(20);
            continue;
          }
        }
    ...
    /* ========== 台下/未确认台上时的初始与重新登台检测 ========== */
    if (onstage_confirmed == 0 && robot_state == ROBOT_RUNNING && control_mode == 1)
    {
      if (is_test_mode == 0)
      {
        if (verification_start_tick == 0)
        {
          verification_start_tick = HAL_GetTick();
          printf("[Verification] onstage_confirmed is 0. Settle timer started (500ms)...\r\n");
        }
        
        if (HAL_GetTick() - verification_start_tick < 500U)
        {
          // 刹车静止等待车体和传感器稳定
          Motor_Control(0, 0);
          osDelay(20);
        ...
        {
          onstage_confirmed = 1;
          verification_start_tick = 0; // 重置
          printf("[Verification] Confirmed ON-STAGE via %s! onstage_confirmed -> 1\r\n", 
                 (low_grey_count >= 3) ? "Grey sensors" : "Geometry");
        }
        else
        {
          robot_state = ROBOT_TEST_ROTATE_PREPARE;
          verification_start_tick = 0; // 重置
          printf("[Verification] Failed to confirm on-stage. Transitioning to ROBOT_TEST_ROTATE_PREPARE to retry...\r\n");
          osDelay(20);
          continue;
        }
      }
    ...
    if (control_mode == 0 && !Control_IsOnline() && !is_test_mode) {
        ...
        if (control_mode == 0) 
        { 
          verification_start_tick = 0; // 遥控模式下重置验证计时器
          /* 在遥控模式下，按下 gamepad Y 键 (buttons & 8) 立即进入自主登台流程（正式模式） */
          if ((ctrl.buttons & 8) && !(prev_buttons & 8))
          {
            is_test_mode = 0;
            robot_state = ROBOT_TEST_ROTATE_PREPARE;
            climb_pd_init = 0; /* 清空 PD 标志 */
            printf("[GamepadTrigger] Y received! Starting Auto Climb Scan...\r\n");
          }
