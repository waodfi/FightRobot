...
void MachineVision_SendTestFrame(void)
{
	...
	uint8_t tx_buf[21];
	tx_buf[0] = MV_FRAME_HEAD_1; // 0xAA
	tx_buf[1] = MV_FRAME_HEAD_2; // 0x55
	tx_buf[2] = MV_TYPE_VISION;  // 0x01
	tx_buf[3] = MV_VISION_PAYLOAD_LEN; // 16
	
	// Payload
	tx_buf[4] = seq++; // seq
	tx_buf[5] = 8;    // tag_id = 8
	tx_buf[6] = 2;    // tag_type = 2
	
	// yaw = 12.34 deg (1234 cdeg = 0x04D2)
	tx_buf[7] = 0xD2;
	tx_buf[8] = 0x04;
	
	// pitch = -5.67 deg (-567 cdeg = 0xFD C9)
	tx_buf[9] = 0xC9;
	tx_buf[10] = 0xFD;
	...
	// CRC-8/MAXIM calculated on bytes from index 2 to 19 (18 bytes total)
	tx_buf[20] = mv_crc8_maxim(&tx_buf[2], 18);
	
	if (s_mv_uart != NULL) {
		HAL_UART_Transmit(s_mv_uart, tx_buf, 21, 100);
	}
}
