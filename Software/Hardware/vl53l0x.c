...
static int check_i2c_comm(vl53l0x_dev_t *dev)
{
    ...
    uint8_t r0 = dev->ll->i2c_read_reg(REF_REG_0);
    uint8_t r1 = dev->ll->i2c_read_reg(REF_REG_1);
    uint8_t r2 = dev->ll->i2c_read_reg(REF_REG_2);

    if (r0 != REF_REG_0_VAL) {
        printf("  REF_REG_0 (0xC0) mismatch: read 0x%02X, expected 0x%02X\r\n", r0, REF_REG_0_VAL);
        ++ret;
    }
    if (r1 != REF_REG_1_VAL) {
        printf("  REF_REG_1 (0xC1) mismatch: read 0x%02X, expected 0x%02X\r\n", r1, REF_REG_1_VAL);
        ++ret;
    }
    if (r2 != REF_REG_2_VAL) {
        printf("  REF_REG_2 (0xC2) mismatch: read 0x%02X, expected 0x%02X\r\n", r2, REF_REG_2_VAL);
        ++ret;
    }
...
static int perform_single_ref_calibration(vl53l0x_dev_t *dev, calibration_type_t calib_type)
{
    ...
    do {
        ...
        dev->ll->delay_ms(2);
        if (timeout_cycles >= 500) { // 500 * 2ms = 1000ms max timeout
            return VL53L0X_FAIL;
        }
        ++timeout_cycles;
    } while (1);
...
static vl53l0x_ret_t vl53l0x_software_reset(vl53l0x_dev_t *dev)
{
    // Write 0x00 to register SOFT_RESET_GO2_SOFT_RESET_N (0xBF) to enter reset
    dev->ll->i2c_write_reg(SOFT_RESET_GO2_SOFT_RESET_N, 0x00);
    
    // Wait until IDENTIFICATION_MODEL_ID (0xC0) reads 0x00
    uint8_t byte = 0xFF;
    int timeout = 0;
    do {
        byte = dev->ll->i2c_read_reg(IDENTIFICATION_MODEL_ID);
        if (byte == 0x00) {
            break;
        }
        dev->ll->delay_ms(2);
        timeout++;
    } while (timeout < 50); // 100ms max

    // Release reset (write 0x01)
    dev->ll->i2c_write_reg(SOFT_RESET_GO2_SOFT_RESET_N, 0x01);

    // Wait until IDENTIFICATION_MODEL_ID (0xC0) reads 0xEE (boot complete)
    timeout = 0;
    do {
        byte = dev->ll->i2c_read_reg(IDENTIFICATION_MODEL_ID);
        if (byte == 0xEE) {
...
{
    if (check_args(dev)) {
        printf("vl53l0x_init: check_args failed\r\n");
        return VL53L0X_FAIL;
    }

    /* HW reset if XSHUT pins are configured, otherwise do SW reset */
    if (dev->ll->xshut_set && dev->ll->xshut_reset) {
        ...
        printf("vl53l0x_init: Performing Software Reset...\r\n");
        vl53l0x_software_reset(dev);
    }

    if (check_i2c_comm(dev)) {
        printf("vl53l0x_init: check_i2c_comm failed\r\n");
        return VL53L0X_FAIL;
    }
    printf("vl53l0x_init: check_i2c_comm OK\r\n");

    if (data_init(dev)) {
        printf("vl53l0x_init: data_init failed\r\n");
        return VL53L0X_FAIL;
    }
    printf("vl53l0x_init: data_init OK\r\n");

    if (static_init(dev)) {
        printf("vl53l0x_init: static_init failed\r\n");
        return VL53L0X_FAIL;
    }
    printf("vl53l0x_init: static_init OK\r\n");
    ...
    if (perform_ref_calibration(dev)) {
        printf("vl53l0x_init: perform_ref_calibration failed\r\n");
        return VL53L0X_FAIL;
    }
    printf("vl53l0x_init: perform_ref_calibration OK\r\n");
